    ...
    <div className="py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {brands.map((brand) => (
            <div key={brand.id} className="glass-panel h-40 flex items-center justify-center rounded-lg hover:bg-white/5 transition duration-300 group cursor-pointer border border-white/5 hover:border-accent/50 p-4">
              {brand.logo_url ? (
                 <img 
                  src={getImageUrl(brand.logo_url)} 
                  alt={brand.name} 
                  className="max-w-full max-h-full object-contain opacity-60 group-hover:opacity-100 transition duration-300 grayscale group-hover:grayscale-0"
